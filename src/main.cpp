...
uint8_t DATA[8] = {};
float speed = 0;
float speed_change = -0.17;
float rotation_change = -0.06;
//ジャンプのための出力
float jump = 0;
...
void canSend()
{
    ...
    while (1)
    {
        ...
        // PWM値を計算
        pwm[0] = speed_change * speed * cos(theta - 2 * M_PI / 3) + (rotation_change * R_x);
        pwm[1] = speed_change * speed * cos(theta - 4 * M_PI / 3) + (rotation_change * R_x);
        pwm[2] = speed_change * speed * cos(theta) + (rotation_change * R_x);
        ...
        // ロボマスのid:3
        int16_t outputDownInt16 = static_cast<int16_t>(pwm[2]);
        DATA[4] = outputDownInt16 >> 8;   // MSB
        DATA[5] = outputDownInt16 & 0xFF; // LSB

        int16_t outputJumpInt16 = static_cast<int16_t>(jump);
        DATA[6] = outputJumpInt16 >> 8;   // MSB
        DATA[7] = outputJumpInt16 & 0xFF; // LSB

        printf("pwm[0]: %d ,pwm[1]: %d ,pwm[2]: %d ,jump: %f\n" ,outputRightInt16 ,outputLeftInt16 ,outputDownInt16, jump);
